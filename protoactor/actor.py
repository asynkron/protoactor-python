...
from protoactor.dispatcher import AbstractMessageInvoker, AbstractDispatcher, ThreadDispatcher
from protoactor.futures import FutureProcess
from protoactor.guardians import Guardians
from protoactor.invoker import AbstractInvoker
...
from protoactor.supervision import AbstractSupervisorStrategy
...
class LocalContext(AbstractContext, AbstractInvoker, AbstractSupervisor, AbstractMessageInvoker):
...
def default_spawner(id: str, props: 'Props', parent: PID) -> PID:
    _context = LocalContext(props.producer, props.supervisor, props.middleware_chain, parent)
    mailbox = props.produce_mailbox(_context, props.dispatcher)
    _process = LocalProcess(mailbox)

    pid = ProcessRegistry().add(id, _process)
